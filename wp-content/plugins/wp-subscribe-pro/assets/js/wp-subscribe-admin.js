...
( function( $ ){
	...
	function initColorPicker( widget ) {
		...
		widget.find('.wp-subscribe-service-field select').change(function() {
			...
			// Thanks Page option
			var thanksService = [ 'activecampaign', 'benchmark', 'constantcontact', 'madmimi', 'mailchimp', 'mailrelay', 'getresponse', 'mailerlite' ];
			widget.find('.wp_subscribe_thanks_page').toggle( thanksService.includes( $this.val() ) );
		}).trigger('change');

		widget.find('.wp_subscribe_include_name input').change(function() {
			$('.wp-subscribe-name_placeholder-field').toggle( $(this).is(':checked') );
		}).trigger('change');

		widget.find('.thanks-page-field').change(function() {
			$('.wp_subscribe_thanks_page_details').toggle( $(this).is(':checked') );
		}).trigger('change');
	}
