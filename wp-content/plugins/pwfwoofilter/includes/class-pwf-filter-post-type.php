...
if ( ! class_exists( 'Pwf_Filter_Post_Type' ) ) {
	...
	class Pwf_Filter_Post_Type {
		...
		*/
		...
		private static $is_plugin_activated = false;
		private static $is_development_site = false;
		...
		public function init() {
			...
			self::$is_network_plugin   = self::check_is_network_plugin();
			self::$site_url            = self::set_site_url();
			self::$is_plugin_activated = self::check_is_plugin_activated();
			self::$is_development_site = self::check_is_development_site();
		}
		...
		public static function plugin_version() {
			$version = get_option( 'pwf_woocommerce_version', null );
			if ( null === $version || version_compare( $version, PWF_WOO_FILTER_VER, '<' ) ) {
		...
		private static function update_purchase_code() {
			...
			if ( ! empty( $purchase_code ) ) {
				...
				update_option( 'envato_purchase_code_28181010', '' );
			}
		}

		public static function get_purchase_code() {
			if ( self::$is_network_plugin ) {
		 ...
		 * @since 1.4.4
		 */
		public static function is_network_plugin() {
		...
		public static function is_plugin_activated() {
			return self::$is_plugin_activated;
		}
		...
		public static function is_development_site() {
			return self::$is_development_site;
		}
		 ...
		 * @since 1.4.4
		 */
		private static function check_is_network_plugin() {

			if ( is_null( self::$is_network_plugin ) ) {
				// Check is PWF as network plugin
				if ( is_multisite() && ( is_plugin_active_for_network( self::$plugin_name ) || is_network_only_plugin( self::$plugin_name ) ) ) {
					return true;
				}
			}
		 ...
		 * @since 1.4.4
		 */
		private static function set_site_url() {
			if ( self::$is_network_plugin ) {
		...
		private static function check_is_plugin_activated() {
		...
		private static function check_is_development_site() {
			$host = self::$site_url;
