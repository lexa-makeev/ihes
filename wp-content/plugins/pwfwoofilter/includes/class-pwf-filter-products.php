...
if ( ! class_exists( 'Pwf_Filter_Products' ) ) {
	...
	class Pwf_Filter_Products {
		 ...
		 * @since 1.3.3
		 */
		protected static $parse_query_vars;
		...
		public function __construct( Pwf_Parse_Query_Vars $query_vars, $attributes = array() ) {
			self::$parse_query_vars = $query_vars;
		...
		protected function parse_attributes( $attributes ) {
			$filter_setting = self::$parse_query_vars->get_filter_setting();
		...
		protected function parse_query_args() {
			...
			$query_args['tax_query']  = self::$parse_query_vars->get_tax_query();
			$query_args['meta_query'] = self::$parse_query_vars->get_meta_query();

			if ( ! empty( self::$parse_query_vars->get_date_query() ) ) {
				$query_args['date_query'] = self::$parse_query_vars->get_date_query();
			}

			if ( ! empty( self::$parse_query_vars->get_search_query() ) ) {
				$query_args['s'] = self::$parse_query_vars->get_search_query();
			}
		...
		protected function product_loop() {
			...
			if ( $products && $products->ids ) {
				...
				foreach ( $products->ids as $product_id ) {
					...
					do_action( 'pwf_before_shop_loop_item', $this->filter_id, self::$parse_query_vars );
		...
		public function get_query_results() {
			$selected_items   = self::$parse_query_vars->selected_items();
		...
		protected function before_products_loop() {
			...
			if ( self::$parse_query_vars->is_stock_status_active() ) {
				add_filter( 'posts_where', array( $this, 'set_stock_status_product_variations' ), 10, 2 );
			}
			if ( self::$parse_query_vars->has_on_sale() ) {
		...
		public function set_stock_status_product_variations( $where, $query_obj ) {
			$product_ids  = self::$parse_query_vars->get_out_of_stock_ptoduct_variations_ids();
			$stock_status = self::$parse_query_vars->get_current_stock_status();
		...
		protected function get_transient_name() {
			...
			/**
			 * Fix transient name cached when query has price, on sale, out of stock variations
			 */
			if ( $this->has_price_item ) {
			...
			if ( self::$parse_query_vars->is_stock_status_active() ) {
				...
				$query_args[ 'pwf_post_stock_' . $stock_status ] = self::$parse_query_vars->get_out_of_stock_ptoduct_variations_ids();
			}

			if ( self::$parse_query_vars->has_on_sale() ) {
				$query_args['pwf_on_sale'] = wc_get_product_ids_on_sale();
			}
			// End fix cache.

			$query_args     = apply_filters( 'pwf_woo_products_loop_transient_name', $query_args, $this->filter_id );
