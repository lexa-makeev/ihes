...
if ( ! class_exists( 'YITH_WACP_Exclusions_Handler' ) ) {
	...
	class YITH_WACP_Exclusions_Handler {
		...
		public function delete_exclusion_prod() {
			...
			$key        = array_search( sanitize_text_field( $_GET['remove_prod_exclusion'] ), $exclusions ); // phpcs:ignore
			if ( false !== $key ) {
		...
		public function search_products_ajax() {
			...
			if ( $posts ) {
				foreach ( $posts as $post ) {
					...
					if ( in_array( $post, $excluded ) ) { // phpcs:ignore
						continue;
					}
		...
		public function delete_exclusion_cat() {
			...
			$key        = array_search( sanitize_text_field( $_GET['remove_cat_exclusion'] ), $exclusions ); // phpcs:ignore
			if ( false !== $key ) {
				unset( $exclusions[ $key ] );
			}
