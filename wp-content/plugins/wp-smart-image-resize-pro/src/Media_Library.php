  ...
  public function enqueue_scripts_filter_dropdown_grid_view()
  {
    ...
    wp_localize_script('wp-smart-image-resize', 'sir_vars', array(
      'filter_strings'     => array(
        'all'     => 'Smart Resize: Все изображения',
        'processed' => 'Smart Resize: Переработано',
        'unprocessed' => 'Smart Resize: Не обработано',
      )
    ));
  }
...
?>
    ...
    <label for="wp-sir-filter" class="screen-reader-text">
      Фильтр по состоянию Smart Image Resize
    </label>
    <select class="wp-sir-filters" name="wp_sir_filter" id="wp-sir-filter">
      <option value="" <?php selected($selected_filter, ''); ?>>Интеллектуальное изменение размера: Все изображения</option>
      <option value="processed" <?php selected($selected_filter, 'processed'); ?>>Smart Resize: Переработано</option>
      <option value="unprocessed" <?php selected($selected_filter, 'unprocessed'); ?>>Smart Resize: Не обработано</option>
    </select>
